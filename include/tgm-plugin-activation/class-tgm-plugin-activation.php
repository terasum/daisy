...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		 ...
		 * @var string
		 */
		public $menu = 'puma-install-plugins';
		 ...
		 * does the puma_init action hook, and hooks in the interactions to init.
		 *
		 * @since 1.0.0
		 *
		...
		protected function __construct() {
			...
			do_action_ref_array( 'puma_init', array( $this ) );
		...
		public function init() {
			...
			$this->strings = array(
				...
				'installing'                      => __( 'Installing Plugin: %s', 'puma' ),
				'oops'                            => __( 'Something went wrong.', 'puma' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'puma' ),
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'puma' ),
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'puma' ),
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'puma' ),
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'puma' ),
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'puma' ),
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'puma' ),
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'puma' ),
				...
				'dashboard'                       => __( 'Return to the dashboard', 'puma' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'puma' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'puma' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'puma' ),
				'dismiss'                         => __( 'Dismiss this notice', 'puma' ),
			);

			do_action( 'puma_register' );
		...
		public function thickbox() {
			...
			if ( ! get_user_meta( get_current_user_id(), 'puma_dismissed_notice_' . $this->id, true ) ) {
				add_thickbox();
			}
		...
		protected function add_admin_menu( array $args ) {
			if ( apply_filters( 'puma_admin_menu_use_add_theme_page', true ) ) {
		...
		public function install_plugins_page() {
			...
			if ( ( 'puma-bulk-install' === $plugin_table->current_action() && $plugin_table->process_bulk_actions() ) || $this->do_plugin_install() ) {
				return;
			}
			...
			<div class="puma wrap">
				...
				<form id="puma-plugins" action="" method="post">
					<input type="hidden" name="puma-page" value="<?php echo esc_attr( $this->menu ); ?>" />
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET['plugin'] ) && ( isset( $_GET['puma-install'] ) && 'install-plugin' === $_GET['puma-install'] ) ) {
				check_admin_referer( 'puma-install' );
				...
				$fields = array( 'puma-install' ); // Extra fields to pass to WP_Filesystem.
				...
				if ( empty( $complete ) ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'puma' ) . '</a>' ), '</p>';
			...
			elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['puma-activate'] ) && 'activate-plugin' === $_GET['puma-activate'] ) ) {
				check_admin_referer( 'puma-activate', 'puma-activate-nonce' );
		...
		public function notices() {
				...
				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					...
					if ( current_user_can( 'install_plugins' ) ) {
						if ( isset( $plugin['required'] ) && $plugin['required'] ) {
							$message['notice_can_install_required'][] = $plugin['name']. 'AAA';
						}
						// This plugin is only recommended.
						else {
							$message['notice_can_install_recommended'][] = $plugin['name'].'CCC';
						}
					}
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_groups ) {
					...
					$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'puma' ), $imploded, $count ) . '</p>';
				}

				// Setup variables to determine if action links are needed.
				$show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'puma' ) . '</a>' : '';
				$show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'puma' ) . '</a>'  : '';

				// Define all of the action links.
				$action_links = apply_filters(
					'puma_notice_action_links',
					array(
						'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
						'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
						'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'puma-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
					)
				);

				$action_links = array_filter( $action_links ); // Remove any empty array items.
				if ( $action_links ) {
					$rendered .= apply_filters( 'puma_notice_rendered_action_links', '<p>' . implode( ' | ', $action_links ) . '</p>' );
				}

				// Register the nag messages and prepare them to be processed.
				$nag_class = version_compare( $this->wp_version, '3.8', '<' ) ? 'updated' : 'update-nag';
				if ( ! empty( $this->strings['nag_type'] ) ) {
					add_settings_error( 'puma', 'puma', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
				}
				else {
					add_settings_error( 'puma', 'puma', $rendered, $nag_class );
				}
			}
		...
		protected function display_settings_errors() {
			...
			settings_errors( 'puma' );

			foreach ( (array) $wp_settings_errors as $key => $details ) {
				if ( 'puma' === $details['setting'] ) {
		...
		public function dismiss() {
			...
			if ( isset( $_GET['puma-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'puma_dismissed_notice_' . $this->id, 1 );
			}
		 ...
		 */
		protected function is_puma_page() {
		...
		public function update_dismiss() {
			...
			delete_metadata( 'user', null, 'puma_dismissed_notice_' . $this->id, null, true );
	...
	if ( ! function_exists( 'load_tgm_plugin_activation' ) ) {
		...
		function load_tgm_plugin_activation() {
			$GLOBALS['puma'] = TGM_Plugin_Activation::get_instance();
		}
	}
...
if ( ! function_exists( 'puma' ) ) {
	 ...
	 */
	function puma( $plugins, $config = array() ) {
		$instance = call_user_func( array( get_class( $GLOBALS['puma'] ), 'get_instance' ) );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function __construct() {
			$this->puma           = call_user_func( array( get_class( $GLOBALS['puma'] ), 'get_instance' ) );
			$this->admin_page_base = $this->puma->parent_slug;
		...
		protected function _gather_plugin_data() {
			...
			foreach ( $this->puma->plugins as $plugin ) {
				if ( is_plugin_active( $plugin['file_path'] ) || ( isset( $plugin['is_callable'] ) && is_callable( $plugin['is_callable'] ) ) ) {
				...
				if ( ! empty( $plugin['source'] ) ) {
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
						...
						$table_data[ $i ]['source'] = __( 'Private Repository', 'puma' );
					}
					else {
						// The plugin is pre-packaged with the theme.
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'puma' );
					}
				}
				// The plugin is from the WordPress repository.
				else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'puma' );
				}

				$table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'puma' ) : __( 'Recommended', 'puma' );

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'puma' ) );
				}
				elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'puma' ) );
				}

				$table_data[ $i ]['file_path'] = $plugin['file_path'];
				$table_data[ $i ]['url']       = isset( $plugin['source'] ) ? $plugin['source'] : 'repo';

				$table_data[ $i ] = apply_filters( 'puma_table_data_item', $table_data[ $i ], $plugin );
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			...
			foreach ( $this->puma->plugins as $plugin => $values ) {
				if ( $name === $values['name'] && isset( $values[ $data ] ) ) {
					return $values[ $data ];
				}
			}
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				...
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s">' . esc_attr_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'puma' ) . '</a>',
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				$activate_url = add_query_arg(
					array(
						...
						'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
						'plugin_source'        => urlencode( $item['url'] ),
						'puma-activate'       => 'activate-plugin',
						'puma-activate-nonce' => urlencode( wp_create_nonce( 'puma-activate' ) ),
					),
					self_admin_url( $this->admin_page_base )
				);
				...
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s">' . esc_attr_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'puma' ) . '</a>',
			...
			$actions = apply_filters( "puma_{$prefix}plugin_action_links", array_filter( $actions ), $item['slug'] );
		...
		public function column_cb( $item ) {
			...
			if ( __( 'Private Repository', 'puma' ) === $item['source'] ) {
				// Escape external URLs
				$plugin_url = esc_url( $plugin_url );
			}
			elseif ( __( 'Pre-Packaged', 'puma' ) === $item['source'] ) {
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'puma' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'puma' ),
				'source' => __( 'Source', 'puma' ),
				'type'   => __( 'Type', 'puma' ),
				'status' => __( 'Status', 'puma' ),
			);

			return apply_filters( 'puma_table_columns', $columns );
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'puma-bulk-install'  => __( 'Install', 'puma' ),
				'puma-bulk-activate' => __( 'Activate', 'puma' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'puma-bulk-install' === $this->current_action() ) {
				...
				if ( empty( $plugin_installs ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'puma' ), '</p></div>';
					return false;
				}
				...
				// Loop through each plugin to install and try to grab information from WordPress API, if not create 'puma-empty' scalar.
				$i = 0;
				foreach ( $plugin_installs as $plugin ) {
					$temp = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) );

					$api[ $i ] = (object) 'puma-empty';
					if ( ! is_wp_error( $temp ) ) {
						$api[ $i ] = $temp;
					}
					$i++;
				...
				if ( is_wp_error( $api ) ) {
					if ( WP_DEBUG === true ) {
						wp_die( esc_html( $this->puma->strings['oops'] ) . var_dump( $api ) ); // wpcs: xss ok
					}
					else {
						wp_die( esc_html( $this->puma->strings['oops'] ) );
					}
				}
				...
				$url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( $this->puma->menu ) ), self_admin_url( $this->admin_page_base ) ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;
			...
			if ( 'puma-bulk-activate' === $this->current_action() ) {
				check_admin_referer( 'bulk-' . $this->_args['plural'] );
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'puma' ), '</p></div>';
					return false;
				}
				...
				if ( is_wp_error( $activate ) ) {
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'puma' ) ), wp_kses_post( $imploded ) );
				}
 ...
 * @since 2.2.0
 */
add_action( 'admin_init', 'puma_load_bulk_installer' );
if ( ! function_exists( 'puma_load_bulk_installer' ) ) {
	function puma_load_bulk_installer() {
		// Get TGMPA class instance
		$puma_instance = call_user_func( array( get_class( $GLOBALS['puma'] ), 'get_instance' ) );


		if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && $puma_instance->menu === $_GET['page'] ) ) {
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function install_strings() {
						...
						$this->strings['no_package']          = __( 'Install package not available.', 'puma' );
						$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'puma' );
						$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'puma' );
						$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'puma' );
						$this->strings['process_failed']      = __( 'Plugin install failed.', 'puma' );
						$this->strings['process_success']     = __( 'Plugin installed successfully.', 'puma' );
					...
					public function activate_strings() {
						...
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'puma' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'puma' );
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'puma' );
						$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'puma' );

						// Automatic activation strings.
						if ( $this->puma->is_automatic ) {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'puma' );
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'puma' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'puma' ) . '</span><span class="hidden">' . __( 'Hide Details', 'puma' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'puma' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'puma' );
						}
						// Default installation strings.
						else {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'puma' );
							$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'puma' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'puma' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'puma' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'puma' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'puma' );
						}
					...
					public function bulk_footer() {
						...
						foreach ( $this->puma->plugins as $plugin ) {
							if ( ! is_plugin_active( $plugin['file_path'] ) ) {
								echo '<p><a href="', esc_url( add_query_arg( 'page', urlencode( $this->puma->menu ), self_admin_url( $this->puma->parent_slug ) ) ), '" target="_parent">', esc_html( $this->puma->strings['return'] ), '</a></p>';
								$complete[] = $plugin;
								break;
							}
							// Nothing to store.
						...
						if ( empty( $complete ) ) {
							echo '<p>', sprintf( esc_html( $this->puma->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'puma' ) . '</a>' ), '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
